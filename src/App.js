...
function App() {
  ...
  const handleNextPage = ()=>{
    if(page === 5){
      fetch("http://localhost:3001/rental/v1/vehicles",{
        method:"post",
        headers:{
      ...
      fetch(`http://localhost:3001/rental/v1/vehicles?wheels=${data.wheels}`)
      .then((res)=>res.json())
      .then((data)=>{
        setFechedData(data)
        setPage((old)=>{
        ...
        <div className='row '>
          <div className='col-md-6'>
              <div className='right-container'>
                <h3 className='text-black '>The Secret To Getting Ahead Is To Get Started </h3>
                <h1 className='text-white'>DON'T DREAM IT DRIVE IT</h1>
              </div>
          </div>
